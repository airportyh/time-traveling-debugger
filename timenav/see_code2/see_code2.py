...
class SeeCode2:
    def __init__(self, filename, code_file_id, line_no):
    ...
    def run(self):
        code_file = self.cache.get_code_file(self.code_file_id)
        filename = get_filename(code_file)
        code_lines = self.cache.get_code_lines(self.code_file_id)
        line = code_lines[self.line_no - 1].strip()
        snapshots = self.nav.get_hits(self.code_file_id, self.line_no)
        
        print("File: %s" % sstring(filename, [CYAN]))
        print("Line %d: %s" % (self.line_no, sstring(line, [CYAN])))
        print("Number of snapshots: %s" % sstring(str(len(snapshots)), [CYAN]))
        
        refs = find_refs(line)
        attributes_to_use = self.get_distinguishing_attributes(snapshots, refs)
        
        # print("attributes_to_use", attributes_to_use)
        
        print("Snapshots:")
        for snapshot in snapshots:
            print("%s" % sstring(str(snapshot["id"]), [CYAN]), end=" ")
            ...
            vars_values = self.get_vars_values(locals_id, snapshot["id"], map(lambda r: r[0], refs))
            
            attrs["variables"] = {}
            
            for var_value in vars_values:
                attrs["variables"][var_value["key"]] = self.fetch_value(var_value["value"], snapshot["id"], 2)
            ...
            if "context" in display_grouped:
                print("context: %s" % sstring(display_grouped["context"], [YELLOW]), end=" ")
            ...
            if "variables" in display_grouped:
                print_variables(display_grouped["variables"])
